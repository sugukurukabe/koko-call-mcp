# KokoCallMCP — 官公需入札MCP Cursor向け設計書 v1.0
...
日本全国の入札情報を Claude / Cursor / VS Code から自然言語で検索・追跡できる MCP サーバ。「鹿児島県の今月のIT入札」「東京都の建設工事で締切が来週のもの」を一発で返す。米国 SAM.gov MCP（`blencorp/capture-mcp-server`）の日本版に相当し、**世界で誰も作っていない**。
...
```mdc
---
description: KokoCallMCP project overview
alwaysApply: true
---
...
\## あなたの役割
このリポジトリは KokoCallMCP（官公需入札MCP）。MCP公式仕様（https://modelcontextprotocol.io）と
...
export async function searchKkj(params: KkjSearchParams): Promise<KkjSearchResult> {
  ...
  const res = await fetch(url.toString(), {
    headers: { "User-Agent": "KokoCallMCP/0.1.0" },
    // 10秒タイムアウト
...
if (mode === "stdio") {
  ...
  app.listen(port, () => {
    console.log(\`KokoCallMCP listening on http://localhost:\${port}/mcp\`);
  });
}
```
...
```bash
mkdir koko-call-mcp && cd koko-call-mcp
git init
cursor .
```
...
```
このリポジトリは KokoCallMCP（官公需入札MCP）の初期化です。
...
| 観点 | KokoCallMCP（本書） | SuguAgriField |
|---|---|---|
| 完成スピード | **5-10日** | 8-12週 |
