...
中小企業庁の官公需情報ポータルサイト（KKJ, Kankouju Kanren Joho）では、官公需の公開入札公示データを横断検索できます。一次情報への入口はあるのに、現場の入札担当者は、案件 1 件あたり 2 時間以上を「調査」だけに費やしてきました。理由は三つあります。
...
「MCP（Model Context Protocol）」というのは、Anthropic が 2024 年 11 月に公開したオープンプロトコルの名前で、AI に外部のデータと操作を渡す方法を標準化するための共通仕様です。現在は複数の AI・開発ツールのエコシステムでサポートが広がっています。
...
次に、ランキングをお願いします。すると Claude は、いきなりランク付けを実行する前に、判定に必要な追加情報を聞き返してきます。ツールの入力スキーマに沿って、専門領域・地域希望・資格情報を対話の中で確定していきます。
...
ChatGPT 側で MCP / Remote connector が利用できる環境では、MCP サーバー URL として `https://mcp.bid-jp.com/mcp` を登録します。利用可否と設定画面は、契約プランや提供時期によって変わるため、ChatGPT 側の最新案内を確認してください。
...
2026 年 9 月末までのベータ期間中は、17 ツールすべてを無料で使えます。これは、入札調査の現場で実際にどう使われるかを知りたいという理由から、価格をいったん引いた状態で公開しているためです。
...
A. `rank_bids` ツールに自社プロファイル（業種・資格・キーワード・予算レンジ）を渡せます。通常のランキング入力は永続保存しません。保存検索を使う場合だけ、検索条件はサーバーの短期メモリ上に保持されます。

### Q. データはサーバーに保存されますか？
A. 通常の検索クエリ・参照した入札・自社プロファイルは永続保存しません。OAuth 2.0 はJWTベースで扱い、保存検索や短期キャッシュなど一部の機能は Cloud Run インスタンスのメモリ上に保持される場合があります。詳しくは[プライバシーポリシー](https://mcp.bid-jp.com/privacy)をご覧ください。
...
A. TypeScript + Express + MCP TypeScript SDK + Vertex AI（PDF 抽出）+ Google Cloud Run。OAuth 2.0 Authorization Code + PKCE + Dynamic Client Registration を実装しています。MCP 2025-11-25 仕様に基づき、Tools / Resources / Resource Templates / Prompts / Completion を組み合わせて、入札検索に必要な文脈を提供しています。詳しくは [GitHub リポジトリ](https://github.com/sugukurukabe/koko-call-mcp) をご覧ください。
