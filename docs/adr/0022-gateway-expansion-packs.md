...
Public MCP JP Gateway（ADR-0016）は現在 7 本の child MCP を束ねている: JP Bids、J-Grants、AgriOps、不動産インテル、法人番号、freee、MoneyForward。GMO銀行系APIは公開 Gateway では提供せず、利用許諾とAPI取得が完了した後の private connector として扱う。しかし Gateway の価値は child MCP の数ではなく「業務の流れが完成する組み合わせ」で決まる。
...
Public MCP JP Gateway (ADR-0016) saat ini menggabungkan 7 MCP anak: JP Bids, J-Grants, AgriOps, Real Estate Intel, Nomor Korporasi, freee, dan MoneyForward. API perbankan GMO tidak diekspos di Gateway publik dan diperlakukan sebagai konektor privat masa depan setelah izin dan akses API diperoleh. Namun, nilai Gateway ditentukan oleh alur kerja bisnis yang dapat disusun, bukan jumlah MCP anak.
