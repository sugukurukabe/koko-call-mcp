...
Model Context Protocol（MCP）は、LLM（Large Language Model）に外部のデータと操作を渡す方法を標準化するオープンプロトコルです。Anthropic が 2024 年 11 月にオープンソースとして公開し、現在は Anthropic / OpenAI / Google / Microsoft などのエコシステムで採用が広がっています。
...
- サーバー側は **Tools / Resources / Resource Templates / Prompts / Completion / Logging / Notifications** の 7 つほどのプリミティブを公開する
...
MCP が最小規約のままでいられる理由は、3 つの登場人物（Host / Client / Server）と 7 つほどのプリミティブだけで、4S のすべてを実装可能な形に切り取っているからです。新概念の追加よりも、既存標準（JSON-RPC 2.0）と既存設計言語の組み合わせを優先した設計判断が、無形資産的な掛け合わせの土台になっています。
